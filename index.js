...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Internal is within the scope of nestedFunction
...
function lowerCaseNames(data){
  let lowerCaseNames = [];
  data.map(animal => lowerCaseNames.push(animal.animal_name.toLowerCase()));
...
function lowPopulationAnimals(data){
  let lowPopulationAnimals = data.filter(animal => animal.population < 5)
...
function USApop(data){
  let animalPop = [];
  data.forEach(animal => animalPop.push(animal.population));
  const reducer = (accumulator, currentValue) => accumulator + currentValue;
  let USApop = animalPop.reduce(reducer, 0);
  return USApop;
}
...
console.log(new CuboidMaker(4, 4, 4))
